...
Scaffold a new module inside `Engine/modules/`.
...
| `--modules-path`   | Path to modules directory (default: `./Engine/modules`) |
| `--workflows-path` | Output dir for workflow (default: `./Engine/workflows`) |
...
```bash
sawectl validate-workflow --workflow Engine/workflows/my_flow.yaml --verbose
```
...
```bash
sawectl run --workflow Engine/workflows/demo.yaml --server localhost:8080
```
...
```bash
sawectl init module logger --modules Engine/modules
sawectl init workflow hello_logger --full --modules logger --modules-path Engine/modules --workflows-path Engine/workflows/default
sawectl validate-workflow --workflow Engine/workflows/default/hello_logger.yaml --modules Engine/modules
sawectl run --workflow Engine/workflows/default/hello_logger.yaml --server localhost:8080
```
